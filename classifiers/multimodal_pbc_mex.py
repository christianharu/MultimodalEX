#!/usr/bin/env python3
"""
Train a multimodal PBC4cip empathy classifier on:
...
# Compatibility shim for older libraries (e.g., PBC4cip) with newer NumPy.
...
def build_inputs(df: pd.DataFrame, feature_set: str) -> Tuple[pd.DataFrame, np.ndarray, int]:
    x = build_context_features(df, feature_set)
...
def normalize_predictions(preds) -> np.ndarray:
    pred_series = pd.to_numeric(pd.Series(preds), errors="coerce")
    if pred_series.isna().any():
        raise ValueError("PBC predictions contain non-numeric values.")
    return pred_series.astype(int).to_numpy()
...
def parse_args() -> argparse.Namespace:
    ...
    default_out = os.path.join(script_dir, "PBC4emp", "results", "multimodal_pbc_mex")

    parser = argparse.ArgumentParser(description="Train multimodal PBC4cip empathy classifier (0/1/2).")
    ...
    parser.add_argument("--tree-count", type=int, default=150)
...
def main() -> None:
    ...
    print("Input shape: %s (N, feature_dim=%d)" % (x.shape, context_dim))
    ...
    y_train_df = pd.DataFrame({LABEL_COL: pd.Series(y_train).astype(int).astype(str)})

    pbc = PBC4cip(tree_count=args.tree_count, filtering=False, multivariate=False)
    _ = pbc.fit(x_train, y_train_df)

    y_val_pred = normalize_predictions(pbc.predict(x_val))
    y_test_pred = normalize_predictions(pbc.predict(x_test))
    y_all_pred = normalize_predictions(pbc.predict(x))
    ...
    model_path = os.path.join(args.output_dir, "best_multimodal_pbc_mex_%s.pkl" % feature_tag)
