#!/usr/bin/env python3
"""
Fine-tune a BERT-based multimodal empathy classifier on:
...
def build_context_features(df: pd.DataFrame, feature_set: str) -> np.ndarray:
    if feature_set == "av":
        return df[AV_COLS].to_numpy(dtype=np.float32)
    ...
    if feature_set == "av_mimicry":
        av = df[AV_COLS].to_numpy(dtype=np.float32)
        return np.concatenate([av, mimicry], axis=1)
...
class BertMultimodalDataset(Dataset):
    def __init__(
        self,
        df: pd.DataFrame,
        indices: np.ndarray,
    ...
    def __getitem__(self, idx: int):
        ...
        enc = self.tokenizer(
            text=str(row["speaker_utterance"]),
            text_pair=str(row["listener_utterance"]),
        ...
        item = {
            ...
            "context_features": torch.tensor(self.context_features[row_idx], dtype=torch.float32),
...
class BertWithContextClassifier(nn.Module):
    def __init__(
        self,
        model_name: str,
        context_dim: int,
    ...
    def forward(self, input_ids, attention_mask, context_features, token_type_ids=None):
        ...
        feats = self.context_norm(context_features)
...
def parse_args() -> argparse.Namespace:
    ...
    default_out = os.path.join(script_dir, "PBC4emp", "results", "bert_multimodal_mex")

    parser = argparse.ArgumentParser(description="Fine-tune BERT multimodal empathy classifier (0/1/2).")
...
def main() -> None:
    ...
    context_features = build_context_features(df, args.feature_set)
    context_dim = int(context_features.shape[1])
    print(f"Context feature dim: {context_dim}")
    ...
    print(
        "Split sizes | "
        f"train={len(train_idx)} ({len(train_idx)/len(df):.2%}), "
        f"val={len(val_idx)} ({len(val_idx)/len(df):.2%}), "
        f"test={len(test_idx)} ({len(test_idx)/len(df):.2%})"
    )

    tokenizer = AutoTokenizer.from_pretrained(args.bert_model)
    train_dataset = BertMultimodalDataset(df, train_idx, context_features, tokenizer, args.max_length)
    val_dataset = BertMultimodalDataset(df, val_idx, context_features, tokenizer, args.max_length)
    test_dataset = BertMultimodalDataset(df, test_idx, context_features, tokenizer, args.max_length)
    all_dataset = BertMultimodalDataset(df, np.arange(len(df)), context_features, tokenizer, args.max_length)
    ...
    model = BertWithContextClassifier(
    ...
    class_weights = build_balanced_class_weights(y[train_idx], EXPECTED_LABELS).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        train_m = run_epoch(model, train_loader, criterion, device, optimizer=optimizer, scheduler=scheduler)
    ...
    model_path = os.path.join(args.output_dir, f"best_bert_multimodal_mex_{feature_tag}.pt")
